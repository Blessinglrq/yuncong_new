...
def mark_anchors(anchors, gt_boxes, positive_threshold=0.3,
    ...
    if len(matched_gt_box_indices) < least_pos_num:
        ...
        allowed_positive_anchor_indices = np.where(max_iou > 0.2)[0]   # paper is 0.1
...
def compute_iou(anchors, gt_boxes):
    ...
    x2 = np.minimum(anchors[:, 3], gt_boxes[:, 3])          #compute chong die quyu

    y_zeros = np.zeros_like(y2.shape)
    x_zeros = np.zeros_like(x2.shape)

    intersect = np.maximum((y2 - y1), y_zeros) * np.maximum((x2 - x1), x_zeros)   #compute chongdie mianji
